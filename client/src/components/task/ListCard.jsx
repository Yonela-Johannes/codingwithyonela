...
const ListCard = ({ elem, project }) => {
  ...
  return loading ? (
    ...
    >
      <div className="flex flex-col items-start justify-between">
        {elem?.project_name ? (
          <div
            ...
            className="text-[13px] lg:text-xl cursor-pointer text-clr_alt"
          >
            {elem?.project_name}
          </div>
        ) : (
        ...
        <p className="text-xs lg:text-sm cursor-pointer">
          {formatDate(elem?.created)}
        </p>
      </div>
      <p className="text-xs lg:text-sm cursor-pointer">
        {project
        ...
        {currentUser && currentUser?.account_id == elem?.account_id ? (
          project ? (
            ...
            <div className="flex gap-4 cursor-pointer">
              <div
                onClick={(e) =>
                  handleDelete(project ? elem?.project_id : elem?.task_id)
                }
                className={`${
                  theme == "light" ? "text-bg_opp" : "text-slate-400"
                } w-4 h-4 object-contain  hover:text-clr_alt duration-300`}
                alt="delete"
              >
                <FaTrash size={14} />
              </div>
        ...
        <div
          className={`text-white ${
            elem?.priority == "low"
              ...
              : ""
          } text-xs lg:text-sm rounded-md px-[1px]`}
        >
          {elem?.priority}
        </div>
      </div>
      ...
      currentUser?.account_id == elem?.account_id ? (
        <>
          {project ? (
            <select
              value={project ? elem?.project_status : elem?.task_status}
              onChange={(e) =>
                handleStatusChange(
                  e,
                  project ? elem?.project_id : elem?.task_id
                )
              }
              className={`${
                theme == "light"
