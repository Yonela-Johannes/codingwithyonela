...
const ResponseModal = () =>
{
    ...
    const { selectedPost, response, setResponse } = useContext(ModalContext)
    ...
    const responseHandler = async () =>
    {
        ...
        {
            if ((selectedPost && currentUser && currentUser?.account_id, response))
            {
                const data = {
                ...
                {loading ? (
                    ...
                    <div className="rounded-md p-2 flex-col flex items-start gap-2 md:gap-4 justify-between w-full border duration-200 cursor-pointer">
                        {responses?.map((res) => (
                            ...
                            >
                                <h4 className="text-sm md:text-base">{res?.response}</h4>
                                <div>
                    ...
                    <p className={`${theme == 'light' ? "text-bg_primary" : "text-bg_lightest"} text-base lg:text-lg my-2`}>Be the first to respond <span>Signin</span></p>
                )}
            </div>
        </div>
    )
