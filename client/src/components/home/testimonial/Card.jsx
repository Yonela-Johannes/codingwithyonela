...
const Card = ({ element, theme }) =>
{
    ...
    return (
        <>
        <div onClick={() => (setData(element), setOpen(true))} key={element?.id} className="relative h-full w-[300px] px-4 py-2 pt-12 cursor-pointer">
            <div className={`flex flex-col ${theme == "light" ? "bg-bg_lightest shadow-md" : "bg-bg_grey"} h-[200px] gap-4 rounded-md p-2 lg:p-4 text-md font-thin`}>
                <p className="flex-1 text-sm lg:text-base">{element.message?.slice(0, 60)}...</p>
        ...
        <Modal
          ...
          >
            {data ? (
        <div className="relative h-full w-full px-4 py-2 pt-12">
        <div className={`flex flex-col ${theme == "light" ? "bg-bg_lightest shadow-md" : "bg-bg_grey"} h-[200px] gap-4 rounded-md p-2 lg:p-4 text-md font-thin`}>
            ...
            <div className="flex items-center gap-2 mt-3">
                {[...Array(5)]?.map((_, index) => (
                    <StarIcon
                        key={index}
                        className={`${data?.rating > index ? "" : "fill-muted stroke-muted-foreground"} h-5 w-5 ${theme == "light" ? "fill-clr_alt text-clr_alt" : "fill-cl_primary text-cl_primary"
                            }`}
                    />
                ))}
            </div>
            <div className="flex items-center gap-2">
                ...
                <div className="lg:flex flex-col items-start justify-between">
                    <div>
                        <h6 className={`${theme == "light" ? "text-clr_alt" : "text-cl_primary"} text-sm lg:text-base font-semibold`}>{data?.name}{" "}{data?.lastname}</h6>
                        <span className={`w-full block ${theme == "light" ? "text-bg_primary" : "text-bg_lightest"} text-xs font-normal italic lg:text-sm`}>
                            {data.company}
                        </span>
                    </div>
        ...
        {currentUser && (currentUser?.is_staff || currentUser?.is_admin) ? (
            <select value={data.status} onChange={e => handleStatusChange(e, data?.id)} className={`${theme == "light" ? "text-bg_opp bg-white" : "bg-bg_core rounded-md"} absolute -bottom-10 mt-5`}>
                <option value="" disabled selected hidden>Select status</option>
                <option value="pending">Pending</option>
                <option value="accepted">Accept</option>
            </select>
