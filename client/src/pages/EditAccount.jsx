...
const EditAccount = () =>
{
	const { currentUser, loading, updateSucess } = useSelector((state) => state.user)
	...
	{
		e.preventDefault()
		if (title && !query) return toast('If you in an IT field please provide you GitHub account!')
		if (!username) return ('Error provide username')
		if (!currentUser && currentUser?.id) return toast('You are not authorized to perform this action')
				...
				<div className={`w-full max-w-md lg:p-8 space-y-6 ${theme == "light" ? "text-black" : "text-white"}`}>
					<form className='space-y-4' onSubmit={handleContinue}>
						...
						<div className="flex gap-4">
							...
							<button
								type="submit"
								className={`flex items-center justify-center rounded-none w-full py-2 text-center border-none font-bold text-white ${theme == "light" ? "bg-clr_alt" : "bg-clr_alt"}  ${!username ? 'opacity-[0.5]' : 'opacity-1'}`}
								disabled={!username}
							>
								{loading ? (
									<>
